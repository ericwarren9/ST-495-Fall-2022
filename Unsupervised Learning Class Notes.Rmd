...
fits <- softImpute(Xna,
                   trace=TRUE,
                   type ="svd")
Xapp2 <- fits$u %*% diag(fits$d) %*% t(fits$v)
cor(Xapp2[ismiss], X[ismiss])
plot(X[ismiss],
     Xapp2[ismiss],
     pch = 16)
abline(0, 1)

# To find which approximation is the best
sum((X[ismiss] - Xapp[ismiss]) ** 2)
sum((X[ismiss] - Xapp2[ismiss]) ** 2)
```
...
as the dissimilarity measure. Complete and average linkage tend to yield evenly sized clusters whereas single linkage tends to yield extended clusters to which single leaves are fused one by one.}
\end{figure}
